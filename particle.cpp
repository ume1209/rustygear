...
HRESULT InitParticle(void)
{
	...
	for(int nCntParticle = 0; nCntParticle < MAX_PARTICLE; nCntParticle++)
	{
		...
		g_aParticle[nCntParticle].scale = XMFLOAT3(0.2f, 0.2f, 0.2f);
...
void UpdateParticle(void)
{
...

	{
		...
		// パーティクル発生
		{
			...
			move.y = /*rand() % 300 / 100.0f - g_fHeightBase;*/ cosf(fAngle) * fLength;
			move.z = rand() % 300 / 100.0f - g_fHeightBase; /*cosf(fAngle) * fLength;*/
			...
			// ビルボードの設定
			SetParticle(pos, move, XMFLOAT4(0.0f, 0.2f, 1.0f, 0.85f), fSize, fSize, nLife);
		}
	}
}
