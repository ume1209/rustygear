...
void UpdateMeshWall(void)
{
	...
	for (int Y = 0; Y < pMesh->nNumBlockY; Y++)
	{
		for (int X = 0; X < pMesh->nNumBlockX; X++)
		{
			g_aMeshWall[Y + X].pos.z -= 1.0;
			if (g_aMeshWall[Y + X].pos.z < -50)
			{
				g_aMeshWall[Y + X].pos.z = 0;
			}
		}	
	}
}
